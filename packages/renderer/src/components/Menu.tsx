    ...
    <nav>
      <ul className="w-full text-center relative">
        {Object.entries(menuParams).map(([menuLabel, {label, items}]) => (
          ...
          <button
              className={
                `relative focus:outline-none ${menuOpened.indexOf(menuLabel as MenuLabelsType) === -1 ? 'focus:bg-teal-500 focus:text-zinc-900' : 'focus:bg-yellow-500 focus:text-zinc-900'} bg-zinc-900 text-zinc-200 w-full h-11 py-2 border-b-2 border-b-zinc-700 ${menuOpened.indexOf(menuLabel as MenuLabelsType) === -1 ? 'hover:bg-teal-500 hover:text-zinc-900' : 'hover:bg-yellow-500 hover:text-zinc-900'} rounded-b-sm z-50`
            }
            ...
            <ul
              className={`${menuOpened.includes(menuLabel as MenuLabelsType) ? 'opacity-100' : 'invisible absolute transform -translate-y-[1024px] opacity-0'} right-0 w-full z-10 text-zinc-300 transition-opacity duration-200 ease-linear`}>
              {items.map(({ id, label: subLabel }) => (
                <li
                  title={subLabel}
                  className={`${selectedScreen === id
                    ? 'bg-teal-500 text-zinc-900'
                    : 'bg-zinc-300 text-zinc-900'} border-b-2 border-b-zinc-900 rounded-b-sm cursor-pointer hover:bg-teal-500 hover:text-zinc-900 text-sm`}
                  key={id}
                  >
                  <button
        ...
        <button
          ...
          className={
            `focus:outline-teal-500 focus:outline-4 disabled:cursor-not-allowed absolute -bottom-9 w-full flex items-center justify-center h-9 ${
